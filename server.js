...
app.get("/", requireLogin, async (req, res) => {
  try {
    // Fetch ONLY items for current user
    const itemsResult = await db.query(
      "SELECT * FROM items WHERE user_id = $1 ORDER BY due_date ASC, id ASC",
      [req.session.userId]
    );
    ...
    // Fetch ONLY habits for current user
...
app.post("/edit", requireLogin, async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "UPDATE items SET title = $1, due_date = $2 WHERE id = $3 AND user_id = $4", 
...
app.post("/toggle-habit", requireLogin, async (req, res) => {
  ...
  try {
    // First make sure the habit belongs to the current user
